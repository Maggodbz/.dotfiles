...
class KayaApp(App):
    ...
    def on_mount(self) -> None:
        ...
        self.run_worker(self._gpu_check_worker, thread=True, group="gpu-check")
    ...
    def _gpu_check_worker(self) -> None:
        # nvidia-smi can take a beat, so keep it off the UI thread.
        has_gpu, summary = environment.gpu_status()
        self.call_from_thread(
            self._add_event,
            "observation" if has_gpu else "warning",
            "GPU" if has_gpu else "No GPU",
            summary,
        )
